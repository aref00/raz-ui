...
describe('Running Test for Tayeh Select', () => {
	...
	test('Check Select TyClass', () => {
		const fn = jest.fn()
		render(<Select options={[]} onChange={fn} placeholder="Select Content" tyClass='test' />);
	...
	test('Check Select Label', () => {
		const fn = jest.fn()
		render(<Select options={[]} label="select label" onChange={fn} placeholder="Select Content" tyClass='test' />);
	...
	test('Check Select Change', () => {
		...
		render(<Select options={[{label: 'test-2', value: 'test'}]} onChange={fn} placeholder="Select Content" tyClass='test' />);
		const node = screen.getByRole('textbox');
	...
	test('Check Select Disabled', () => {
		const fn = jest.fn();
		render(<Select disabled options={[{label: 'test', value: 'test'}]} onChange={fn} placeholder="Select Content" tyClass='test' />);
		const node = screen.getByRole('textbox');
	...
	test('Check Select Icon', () => {
		...
		const icon_node = screen.getByRole('img');
		node.click();
	...
	test('Check Select Defaults', () => {
		const fn = jest.fn();
		render(<Select defaultValue={{label: 'test-2', value: 'test'}} options={[{label: 'test', value: 'test'}]} onChange={fn} placeholder="Select Content" tyClass='test' />);
		const input_node = screen.getByRole('textbox');
