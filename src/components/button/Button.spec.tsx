...
describe('Running Test for Tayeh Button', () => {
	...
	test('Check Button with onClick', () => {
		const spy = jest.fn();
		render(<Button onClick={spy} label="Tayeh button"/>);
		const node = screen.queryByRole('button', { name: 'Tayeh button' });
		node?.click();
